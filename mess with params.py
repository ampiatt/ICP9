...
def create_model(x_size, y_size):
    ...
    seq_model.add(Dense(40, activation="Sigmoid", input_shape=(x_size,)))
    seq_model.add(Dense(20, activation="Tanh"))
    seq_model.add(Dense(y_size))
    print(seq_model.summary())
    seq_model.compile(loss='mean_squared_error', optimizer=Adagrad(), metrics=[metrics.mae])
    return seq_model

used_model = create_model(x_train.shape[1], 1)
used_model.summary()

history_model = used_model.fit(x_train, y_train, batch_size=128, epochs=800, shuffle=True, verbose=0, validation_data=(x_test, y_test),)
